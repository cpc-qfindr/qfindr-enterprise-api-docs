...
❓. **MCPサービス経由でQFINDR APIを利用した場合、取得企業数はカウントされますか？**

> ✅. はい、カウントされます。アクセストークン毎に取得企業数が通常通り加算され、上限に達した場合は接続が制限されます。

❓. **MCPサービス経由でQFINDR APIを利用した場合、API利用料は発生しますか？**

> ✅. QFINDR APIを利用するためには、Proライセンスの場合別途オプションのご契約が必要となります。Enterpriseライセンスの場合は、標準で利用可能です。どちらも、オプション料金以外の超過的な費用は発生いたしません。

❓. **生成AIからQFINDR MCPサービスを利用した場合、生成AI側の消費トークンを抑える方法はありますか？**

> ✅. プロンプトに、「リスト出力件数を上位10件に制限し、検索結果URLを併記してください」などの指示を記載することで、生成AI側の消費トークンを抑えることが可能です。また、「toonモードでQFINDR APIを使ってください」と記載することで、QFINDR APIからの情報を圧縮し、トークンを抑えることが可能です。企業検索を伴う場合、「最初に件数を確認してください」と指示を出しておくと安全です。

❓. **Claude DesktopのFreeプランで利用していたら、Claude側の利用上限に達しました。**

> ✅. 無料版の場合は、制限が厳しいため、「新しいチャット」を開いて新規で会話を行うことで、再度利用ができる可能性があります。

❓. **QFINDR APIを直接呼び出す場合と、MCPサービス経由でQFINDR APIを利用した場合で、取得できる情報に違いはありますか？**

> ✅. 一部違いがあります。また、MCPサービス経由でQFINDR APIを利用した場合、生成AIが情報を解釈し、加工するため、取得できる情報が制限される可能性があります。
