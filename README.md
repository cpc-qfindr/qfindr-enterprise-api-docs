...
QFINDR Enterprise APIは、エンタープライズ向けプランをご契約のお客様にご提供しております。  
...
```markdown
QFINDRで群馬県の企業で売上高20億円以下(推定含む)で、設立が浅い企業で水の浄化関連の装置を作っているような会社のリストを出したい。
ある場合は、QFINDRの検索ページへのURLも併記して。
```

🎉結果例：

```markdown
検索結果が見つかりました。群馬県で売上高20億円以下(推定含む)、設立が浅く、水の浄化関連装置を製造している企業を中心に12社が該当しました。
該当企業リスト特に装置製造に該当する主要な3社:
...
```markdown
株式会社クレジット・プライシング・コーポレーション (https://www.credit-pricing.com/) の類似企業を
金融リスクモデリングの観点でQFINDRで探してください。ある場合は、QFINDRの検索ページへのURLも併記して。
```

🎉結果例：

```markdown
株式会社クレジット・プライシング・コーポレーション(CPC)の類似企業を金融リスクモデリングの観点から探すため、
まずCPCの事業内容を確認してから、QFINDRで検索します。
...
Claude Desktopの左下にあるアカウントより「設定」を選択します。

「開発者」タブより、「ローカルMCPサーバー」の「設定を編集」により以下の`claude_desktop_config.json`を編集します。(ファイルが存在しない場合は作成を行ってください。)
...
powershellを用いて以下のコマンドを実行してください。
...
⚠️ Claude Desktopアプリをご利用の場合、設定後タスクマネージャより完全にClaudeアプリを終了させてから再起動してください。
...
#### QFINDR Enterprise APIが利用できるかの確認
...
> [!TIP]
> MCPサービスの活用事例などについての情報を随時更新予定です。

❓. **MCPサービス経由でQFINDR Enterprise APIを利用した場合、取得企業数はカウントされますか？**

> ✅. はい、カウントされます。アクセストークン毎に取得企業数が通常通り加算され、上限に達した場合は接続が制限されます。

❓. **MCPサービス経由でQFINDR Enterprise APIを利用した場合、API利用料は発生しますか？**

> ✅. QFINDR Enterprise APIを利用するためには、Proライセンスの場合別途オプションのご契約が必要となります。Enterpriseライセンスの場合は、標準で利用可能です。どちらも、オプション料金以外の超過的な費用は発生いたしません。

❓. **生成AIからQFINDR MCPサービスを利用した場合、生成AI側の消費トークンを抑える方法はありますか？**

> ✅. プロンプトに、「リスト出力件数を上位10件に制限し、検索結果URLを併記してください」などの指示を記載することで、生成AI側の消費トークンを抑えることが可能です。また、「toonモードでQFINDR Enterprise APIを使ってください」と記載することで、QFINDR Enterprise APIからの情報を圧縮し、トークンを抑えることが可能です。企業検索を伴う場合、「最初に件数を確認してください」と指示を出しておくと安全です。

❓. **Claude DesktopのFreeプランで利用していたら、Claude側の利用上限に達しました。**

> ✅. 無料版の場合は、制限が厳しいため、「新しいチャット」を開いて新規で会話を行うことで、再度利用ができる可能性があります。

❓. **QFINDR Enterprise APIを直接呼び出す場合と、MCPサービス経由でQFINDR Enterprise APIを利用した場合で、取得できる情報に違いはありますか？**

> ✅. 一部違いがあります。また、MCPサービス経由でQFINDR Enterprise APIを利用した場合、生成AIが情報を解釈し、加工するため、取得できる情報が制限される可能性があります。
